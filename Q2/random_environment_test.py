...
class Environment:
    ...
    def __init__(self,  magnification):
        ...
        self.save_im = 'close'  # ['every', 'close'] 
    ...
    def show(self, agent_state, all_states=None, save_im='close'):
        ...
        cv2.rectangle(self.image, window_top_left, window_bottom_right, (90, 90, 90), thickness=cv2.FILLED)
        ...
        if np.any(all_states):
            ...
            for i, state_ in enumerate(all_states):
                ...
                cv2.line(self.image, (int(state[0]), int(self.magnification-state[1])), (int(state_[0]), int(self.magnification-state_[1])), (0,255,0), 2)
